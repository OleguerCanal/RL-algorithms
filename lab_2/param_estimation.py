...
def get_model(nlayers, nunits, lr):
    model = Sequential()
    model.add(Dense(int(nunits), input_dim=4, activation='relu',
                    kernel_initializer='he_uniform'))
    for i in range(int(nlayers)):
        model.add(Dense(int(nunits), activation='relu', kernel_initializer='he_uniform'))
...
def eval_agent(point):
    ...
    agent.train(generate_experiment_name(parameters), episode_num=1000, solved_score=195)
    res = agent.test(300)
